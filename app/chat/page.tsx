...
export default function Home() {
  ...
  const songDescription =
    generation?.song?.scale && generation?.song?.key
      ? `in the key of ${capitalizeFirstLetter(convertToFlat(generation?.song?.key))} ${capitalizeFirstLetter(generation?.song?.scale)}`
      : '';
  ...
  const onSubmit = async (value: string) => {
    ...
    const { object } = await generate({ key, input: value });
